...
[TestFixture]
public class GetSubCountyPointQueryTests
{
    ...
    {
        var res =await _mediator.Send(new GetSubCountyPointQuery(name));
        Assert.That(res.IsSuccess,Is.True);
        Assert.That(res.Value.Any(),Is.True);
        foreach (var f in res.Value)
            Log.Information($"{f.County}|{f.SubCounty}|{f.Count}|{f.Rate}");
    }
}